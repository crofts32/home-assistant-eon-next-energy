...
class EonEnergySummary extends HTMLElement {
  ...
  set hass(hass){this._hass=hass;const signature=Object.values(hass.states).filter(s=>s.attributes?.consumption_statistics?.startsWith("eon_next_energy:")).map(s=>s.last_updated).join();if(signature!==this._signature){this._signature=signature;this._refresh();}}
