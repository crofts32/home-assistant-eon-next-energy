...
const share=(part,total)=>Number.isFinite(part)&&Number.isFinite(total)&&total>0?part/total*100:null;
const percentage=(value)=>value===null?"—":value.toLocaleString("en-GB",{maximumFractionDigits:1})+"%";
...
function normalizeHistoricalCharges(entries=[]) {
  const charges=new Map();
  for(const item of entries) {
    const energy=Number(item?.energy_kwh),rate=item?.rate;
    if(!/^\d{4}-\d{2}-\d{2}$/.test(item?.date||"")||!Number.isFinite(energy)||energy<=0||!["offpeak","peak"].includes(rate))continue;
    charges.set(item.date,{energy,rate});
  }
  return charges;
}
class EonEnergySummary extends HTMLElement {
  setConfig(config) {
    ...
    this._historicalCharges=normalizeHistoricalCharges(config.historical_charges);
  ...
  _evDay(day){
    if(this._chargeDays?.has(day))return {energy:this._chargeDays.get(day),cost:this._chargeCostDays?.get(day),source:"Hypervolt"};
    const historical=this._historicalCharges?.get(day);
    if(historical){
      const tariff=this._freshness?.find(item=>Number.isFinite(item.offpeakRate)&&Number.isFinite(item.peakRate));
      const rate=historical.rate==="offpeak"?tariff?.offpeakRate:tariff?.peakRate;
      return {energy:historical.energy,cost:Number.isFinite(rate)?historical.energy*rate:null,source:"Tesla backfill"};
    }
    return this._manualChargeDays?.has(day)?{energy:null,cost:null,source:"Confirmed"}:null;
  }
  _evTotals(from,to){
    let energy=0,cost=0,energyFound=false,costFound=false;
    for(let day=from;day<=to;day=shiftDay(day,1)){const ev=this._evDay(day);if(Number.isFinite(ev?.energy)){energy+=ev.energy;energyFound=true;}if(Number.isFinite(ev?.cost)){cost+=ev.cost;costFound=true;}}
    return {energy:energyFound?energy:null,cost:costFound?cost:null};
  }
  _chart(totals){
    ...
    days.forEach((day,i)=>{
      const e=totals.electricity.days.get(day),offpeak=totals.electricity.offpeakDays.get(day)||0,peak=totals.electricity.peakDays.get(day)||0,g=totals.gas.days.get(day),ev=this._evDay(day),x=left+i*step+step*.15,bw=step*.7,oh=offpeak/max*plot,ph=peak/max*plot,gh=(g||0)/max*plot;
      const evDetail=Number.isFinite(ev?.energy)?` · EV charging ${number(ev.energy)} kWh · estimated ${money(ev.cost)} · ${ev.source}`:ev?" · Confirmed EV charge day · charger data unavailable":"";
      ...
      if(ev)svg+=`<circle cx="${x+bw/2}" cy="${Math.max(7,bottom-oh-ph-gh-7)}" r="4" class="ev-marker"><title>${displayDate(day)} · ${Number.isFinite(ev.energy)?`EV charging ${number(ev.energy)} kWh · estimated ${money(ev.cost)} · ${ev.source}`:"Confirmed EV charge day · charger data unavailable"}</title></circle>`;
  ...
  _render(){
    ...
    const selectedEv=ready?this._evTotals(this._from,this._to):null;
    const monthEv=ready?this._evTotals(monthStart(now),now):null;
    ...
    const selectedOffpeak=ready?sumValues(totals.electricity.offpeakDays.values()):null;
    const selectedPeak=ready?sumValues(totals.electricity.peakDays.values()):null;
    ...
    :host{display:block;color:var(--primary-text-color);font-family:var(--paper-font-body1_-_font-family,system-ui)}ha-card{padding:24px;border-radius:18px}h1{font-size:25px;margin:0 0 4px}h2{font-size:18px;margin:26px 0 12px}.muted{color:var(--secondary-text-color);font-size:13px;line-height:1.6}.controls{display:flex;gap:10px;flex-wrap:wrap;align-items:end;margin:20px 0 12px}label{font-size:12px;display:grid;gap:5px}input,button{font:inherit;color:inherit;background:var(--secondary-background-color);border:1px solid var(--divider-color);border-radius:8px;padding:9px 12px}button{cursor:pointer}button:disabled{opacity:.45;cursor:wait}.chips{display:flex;gap:8px;flex-wrap:wrap}.cards{display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin:12px 0}.live-cards{grid-template-columns:repeat(2,1fr)}.metric{padding:16px;border:1px solid var(--divider-color);border-radius:12px}.metric strong{display:block;font-size:26px;margin-top:8px}.range{font-size:16px;font-weight:600;margin:18px 0}.legend{display:flex;gap:20px;flex-wrap:wrap;font-size:13px}.tariff-summary{display:flex;gap:10px;flex-wrap:wrap;margin:14px 0 4px}.tariff-metric{display:flex;align-items:center;gap:9px;padding:9px 12px;border:1px solid var(--divider-color);border-radius:10px;font-size:13px}.tariff-metric strong{font-size:16px}.dot{display:inline-block;width:10px;height:10px;border-radius:3px;margin-right:6px;flex:none}.electricity-peak,.peak-dot{fill:#53a9f7;background:#53a9f7}.electricity-offpeak,.offpeak-dot{fill:#3657d6;background:#3657d6}.gas,.gas-dot{fill:#f1a55b;background:#f1a55b}.ev-marker,.ev-dot{fill:#b66cff;background:#b66cff}.ev-dot{border-radius:50%}.missing{fill:var(--disabled-text-color)}svg{width:100%;height:auto}svg text{fill:var(--secondary-text-color);font-size:11px}.grid{stroke:var(--divider-color);stroke-width:1}table{width:100%;border-collapse:collapse;font-size:14px}td,th{padding:12px 4px;border-bottom:1px solid var(--divider-color);text-align:right}td:first-child,th:first-child{text-align:left}tfoot{font-weight:700}a{color:var(--primary-color)}.error{padding:18px;color:var(--error-color)}@media(max-width:600px){ha-card{padding:16px}.cards{grid-template-columns:1fr}.metric strong{font-size:23px}.controls input{max-width:145px}}</style>
    ...
    <div class="tariff-summary" aria-label="Electricity tariff split for selected dates"><div class="tariff-metric"><i class="dot offpeak-dot"></i><span>Off-peak <strong>${number(selectedOffpeak)} kWh</strong> · ${percentage(share(selectedOffpeak,totals.electricity.sum))}</span></div><div class="tariff-metric"><i class="dot peak-dot"></i><span>Peak <strong>${number(selectedPeak)} kWh</strong> · ${percentage(share(selectedPeak,totals.electricity.sum))}</span></div></div>
    ${this._chart(totals)}
...
export {londonMidnight, dateKey, shiftDay, summarize, summarizeChargeHistory, summarizeChargeCostHistory, normalizeHistoricalCharges, sumValues, share};
