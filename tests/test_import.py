...
namespace = dict(datetime=datetime, time=__import__('datetime').time, timedelta=timedelta, UTC=UTC,
    LONDON=calculations.LONDON, sha256=sha256, DOMAIN='eon_next_energy', CONF_HISTORY_DAYS='history_days',
    DEFAULT_HISTORY_DAYS=30, CORRECTION_LOOKBACK_DAYS=14, aggregate_complete_hours=calculations.aggregate_complete_hours,
...
class ImportTests(IsolatedAsyncioTestCase):
    async def check_import(self, unit, fuel, prior=None):
        writes=[]
        ...
        owner=SimpleNamespace(hass=None, _last_statistic=AsyncMock(return_value=(prior, 10.0 if prior else 0.0)),
            _sum_before=AsyncMock(return_value=10.0 if prior else 0.0), _setting=lambda k, d:d,
